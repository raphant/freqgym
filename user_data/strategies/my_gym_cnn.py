...
from time_series_to_gaf.cnn_model import create_cnn, predict
from time_series_to_gaf.constants import REPO
from time_series_to_gaf.preprocess import quick_gaf, transform
...
class SagesGymCNN(IStrategy):
    ...
    def __init__(self, config: dict) -> None:
        ...
        model_map = {
            'BTC/USDT': 'computer_vision_model_20220402163509_3_of_3.h5',
            'ETH/USDT': 'computer_vision_model_20220402223415_1_of_1.h5',
        }
        for pair, model_name in model_map.items():
            try:
                model = create_cnn(40)
                model_to_load = REPO / model_name
                try:
                ...
                logger.info(f'Loaded model: {model_to_load}')
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['buy'] = (action > 0.6).astype('int')
        dataframe['sell'] = (action < 0.50).astype('int')
    ...
    def rl_model_predict(self, dataframe: DataFrame, pair: str):
        ...
        for window in range(self.window_size, len(dataframe), self.window_size):
            start = window - self.window_size
            end = window
            observation = self.preprocess(indicators[start:end])
            res = self.models[pair].predict(observation)[0][0]
            action_output.loc[end] = (res - 1) * -1
